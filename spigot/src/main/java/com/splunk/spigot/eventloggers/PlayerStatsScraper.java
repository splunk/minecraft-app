 ...
 * ({@code <world>/players/stats/<uuid>.json} and {@code .../advancements/<uuid>.json}),
 * flattens them into one document per player, and upserts those documents into a Splunk
 * KV-store collection via {@link KvStoreConnection}.
 *
 * <p>This data is intentionally NOT sent through HEC/an index: it is a current snapshot, not
 * a time series, so a KV store keyed by player UUID (idempotent overwrite) is the right home.
 * See {@link KvStoreConnection} for why HEC can't be used.
 *
 * <p>Runs off the main server thread (see {@link ScheduledMetricLogger#startAsync}); it only
 * reads files and does HTTP, never touches the Bukkit world API. To avoid re-uploading
 * unchanged players every cycle, the last-seen file modification time per UUID is cached and
