...
public class DeathEventLogger extends AbstractEventLogger {
  ...
  /**
   * Whether to turn off logging non-player related monster deaths. Monsters causing their own death
   * generates a lot of spammy events. E.g. Bats flying into lava...
   */
  ...
  public void captureDeathEvent(LivingDeathEvent event) {
    ...
    final LoggableDeathEvent.DeathEventAction deathAction =
        playerDied
            ? LoggableDeathEvent.DeathEventAction.PLAYER_DIED
    ...
    final Point3dLong position =
        new Point3dLong(entityPos.xCoord, entityPos.yCoord, entityPos.zCoord);
    ...
    logAndSend(
        new LoggableDeathEvent(deathAction, gameTime, worldName, position)
            .setKiller(killer)
            .setVictim(victim)
