...
public class PlayerEventLogger extends AbstractEventLogger {
  ...
  private final Cache<String, Vec3> lastKnownCoordinates =
      CacheBuilder.newBuilder()
          .maximumSize(MAX_PLAYERS)
          .build(
  ...
  public void onPlayerConnect(PlayerLoggedInEvent event) {
    logAndSend(
        generateLoggablePlayerEvent(
            event, LoggablePlayerEvent.PlayerEventAction.PLAYER_CONNECT, null, null));
  }
  ...
  public void onPlayerChat(ServerChatEvent chatEvent) {
    logAndSend(
        generateLoggablePlayerEvent(
            chatEvent, LoggablePlayerEvent.PlayerEventAction.CHAT, chatEvent.message));
  }

  private static LoggablePlayerEvent generateLoggablePlayerEvent(
      PlayerEvent event,
      LoggablePlayerEvent.PlayerEventAction actionType,
      String reason,
      String message) {
    ...
    final Point3dLong coordinates =
        new Point3dLong(playerPos.xCoord, playerPos.yCoord, playerPos.zCoord);
  ...
  /**
   * Living update seems to get called about 10x/sec. We check if the update belongs to a player and
   * if so we check if the players position has changed significantly based on {@code GRANULARITY}.
   *
  ...
  public void onPlayerStatusReported(LivingUpdateEvent playerMove) {
    if (playerMove.entity instanceof EntityPlayer) {
      ...
      LoggablePlayerEvent playerEvent =
      ...
      if (lastCoords != null) {
        playerEvent.setFrom(
            new Point3dLong(lastCoords.xCoord, lastCoords.yCoord, lastCoords.zCoord));
